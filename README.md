...
- mandatory project-member role assignment: a project creator becomes QA lead, additional Jira users default to QA member but require an explicit role, existing assignments are editable, and Jira administrator access is always derived from live Jira permissions;
...
Qaira never writes a user into Jira's administrator role. `jira-admin` is a live derived access profile and is rejected by membership create/update. New project creation writes the creator as QA lead and requires every additional selected user to carry `qa-lead`, `qa-member`, or `viewer`; configuration failures are returned separately after Jira has created the project so the UI does not falsely report an atomic rollback. Administrative Qaira permissions are available only to the live Jira administrator profile and are not assignable to QA lead, QA member, Viewer, or custom roles.
...
There are 28 registered flags in this release, including `qaira.ai.quality_insights` for portfolio insights/quality-gate assessments and `qaira.ai.bug_triage` for AI bug drafting. These routes additionally require their explicit Qaira permissions, and artifact-scoped previews remain subject to their manual domain feature and permission. Feature requirements are cumulative.

## Performance model

The resolver uses a request-scoped cache for repeated project, identity, permission, registry, and entity-property reads. The cache exists only for one resolver invocation, shares in-flight promises, and is updated or invalidated after writes; it is not a cross-request cache or a source of truth. Jira search requests embed the structured issue properties needed to map requirements, defects, cases, suites, runs, plans, automation assets, objects, and quality gates, avoiding a property request for every search result. Remaining property shards are fetched in bounded parallel batches. Test-case/Design list screens use embedded `step_count`/`step_types` metadata and fetch full steps only for the selected case or selected suite through one project-scoped batch, eliminating the invalid eager all-step request. Test-case authoring submits case metadata and all edited steps in one optimistic-concurrency update. Version snapshots are separate properties so history cannot inflate the live specification property; every property remains under Qaira's 30,000-byte guard.
